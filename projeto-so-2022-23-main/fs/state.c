...
int state_init(tfs_params params) {
    ...
    //pthread_mutex_init(&trinco2,NULL);
...
int find_in_dir(inode_t const *inode, char const *sub_name) {
    //MUTEX AQUI  PQ NAO E OPERACAO ATOMICA
...
int in_open_file_table(char const * name){
    int inumber = find_in_dir(inode_get(ROOT_DIR_INUM),name);
    //pthread_mutex_lock(&trinco2);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (open_file_table[i].of_inumber == inumber && free_open_file_entries[i] == TAKEN){
           // pthread_mutex_unlock(&trinco2);
            return 0;
        }
    }
